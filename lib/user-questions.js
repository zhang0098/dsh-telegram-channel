...
export class TelegramUserQuestions {
    ...
    install(ctx) {
        ...
        if (typeof ctx.on === 'function') {
            this.removeServiceListener = ctx.on('internal/service', listener);
        }
    }
    /**
     ...
     */
    async mirrorAsk(chats, request, original) {
        const tgAbort = new AbortController();
        const signals = [tgAbort.signal];
        if (request.signal)
            signals.push(request.signal);
        const combinedSignal = AbortSignal.any(signals);
        const telegram = this.askViaTelegram(chats, { ...request, signal: combinedSignal })
            .then((answer) => ({ channel: 'telegram', answer }))
            .catch((err) => ({ channel: 'telegram', err: err }));
        const tui = original(request)
            .then((answer) => ({ channel: 'tui', answer }))
            .catch((err) => ({ channel: 'tui', err: err }));
